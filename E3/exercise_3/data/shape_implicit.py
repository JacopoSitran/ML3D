...
class ShapeImplicit(torch.utils.data.Dataset):
    ...
    def get_sdf_samples(self, path_to_sdf):
        ...
        first = np.random.choice(pos_tensor.shape[0],int(self.num_sample_points/2))
        pos_random = pos_tensor[first]
        sec = np.random.choice(neg_tensor.shape[0],int(self.num_sample_points/2))
        neg_random = neg_tensor[sec]
        #print (pos_random.shape,neg_random.shape)
        final = np.concatenate((pos_random,neg_random),axis=0)
