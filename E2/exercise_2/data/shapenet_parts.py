...
class ShapeNetParts(torch.utils.data.Dataset):
    ...
    def get_point_cloud_with_labels(shapenet_id):
        ...
        point_cloud = []
        labels = []
        with open(str(ShapeNetParts.dataset_path / category_id / "points" / shape_id)+".pts", 'r') as file:
            for line in file:
                point_cloud.append(line.split())
        point_cloud = np.asarray(point_cloud).T
        with open(str(ShapeNetParts.dataset_path / category_id / "points_label" / shape_id)+".seg", 'r') as file:
            for line in file:
                labels.append(line)
        labels = np.asarray(labels)
        indexes = np.indices((len(labels),)).squeeze()
        indexes = np.random.choice(indexes,1024)
        ...
        for i in range(len(labels)):
            ...
            labels[i] = ShapeNetParts.part_id_to_overall_id[category_id+"_"+labels[i]]
        return np.asarray(point_cloud,dtype=float), np.asarray(labels,dtype=float)
