...
class SimpleDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,split=None):
        ...
        self.mydata,self.labels = None,None
        if split=="train" :
            self.mydata,self.labels = generate_toy_data(4096)
        else:
            self.mydata,self.labels = generate_toy_data(1024)
    def __getitem__(self, idx):
        curr_item = self.mydata[idx]
        re_curr_item = curr_item.reshape((1,curr_item.shape[0],curr_item.shape[1],curr_item.shape[2]))
        return re_curr_item,self.labels[idx]
...
def main(config):
    ...
    # Create Dataloaders
    train_dataset = SimpleDataset('val')  # TODO Instantiate Dataset in train split
