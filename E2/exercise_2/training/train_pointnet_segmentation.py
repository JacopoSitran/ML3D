...
def train(model, trainloader, valloader, device, config):
    ...
    for epoch in range(config['max_epochs']):
        ...
        for i, batch in enumerate(trainloader):
            ...
            input_data =  batch["points"].float()
            target_labels = batch["segmentation_labels"].long()
            prediction = model(input_data).transpose(2, 1)
            ...
            if iteration % config['validate_every_n'] == (config['validate_every_n'] - 1):
                ...
                for batch_val in valloader:
                    ...
                    with torch.no_grad():
                        input_data =  batch_val["points"].float()
                        target_labels = batch_val["segmentation_labels"].long()
                        prediction = model(input_data).transpose(2, 1)
                    ...
                    loss_val += loss_criterion(prediction, batch_val['segmentation_labels'].long()).item()
