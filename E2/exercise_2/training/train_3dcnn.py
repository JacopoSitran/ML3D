    ...
    for epoch in range(config['max_epochs']):
        ...
        for i, batch in enumerate(trainloader):
            ...
            for output_idx in range(prediction.shape[1]):
                prediction
                loss_total = loss_total + loss_criterion(prediction[:, output_idx, :],target_labels)  # TODO: Loss due to prediction[:, output_idx, :] (output_idx=0 for global prediction, 1-8 local)
            ...
            if iteration % config['validate_every_n'] == (config['validate_every_n'] - 1):
                ...
                for batch_val in valloader:
                    ...
                    with torch.no_grad():
                        ...
                        input_data =  batch_val["voxel"]
                        target_labels = batch_val["label"]
                        prediction = model(input_data)
                    ...
                    loss_total_val += loss_criterion(prediction[:,0,:], target_labels).item()
