        ...
        for d in range(backbone_features.shape[2]):
            for h in range(backbone_features.shape[3]):
                for w in range(backbone_features.shape[4]):
                    ...
                    res = backbone_features[:,:,d,h,w]
                    res = res.reshape((res.shape[0],512))
                    partial_object_prediction = partial_predictor(res)
                    #partial_object_prediction = partial_predictor(backbone_features[d * self.feature_cube_side ** 2 + h * self.feature_cube_side + w])

                    predictions_partial.append(partial_object_prediction)

        # TODO: Get prediction for whole object

        res2 = backbone_features.reshape((backbone_features.shape[0],(512*8)))
        full_prediction = self.full_predictor(res2)
