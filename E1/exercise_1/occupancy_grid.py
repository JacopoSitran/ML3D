...
def occupancy_grid(sdf_function, resolution):
    ...
    # ###############
    # TODO: Implement
    coordinates = np.indices((resolution,resolution,resolution))
    coordinates = coordinates/resolution-0.5
    sdf = sdf_function(coordinates[0],coordinates[1],coordinates[2])
    grid = sdf.reshape((resolution,resolution,resolution))
    grid[grid>0] =0
    grid[grid<0] =1
    return grid
    # ###############
